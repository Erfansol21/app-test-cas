...
public class UserController {
    ...
    // User login
    @PostMapping("/login")
    public String login(@ModelAttribute("userLogin") UserLogin login, HttpSession session, Model model) {
        if (userServices.validateLoginCredentials(login.getUserEmail(), login.getUserPassword())) {
    ...
    // CRUD operations for admin management of users
    @PostMapping("/add")
